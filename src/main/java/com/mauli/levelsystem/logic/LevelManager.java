...
public class LevelManager {
    ...
    public Status getStatus(Player p, int level) {
        ...
        int haveVot = store.getVotes(id); // aktuell aus DataStore; VotingPlugin-Hook könnte hier später einfließen

        boolean ok = haveMin >= needMin && haveVot >= needVot;
        return ok ? Status.AVAILABLE : Status.LOCKED;
    }
